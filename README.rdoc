...
use the compiler key for that module:
...
<tt>path</tt>: The path to the Closure Compiler jar.  This has no
default, and must be given.

<tt>flags</tt>: Flags to pass to Closure Compiler.  Default: no flags.
...
<tt>compilations</tt>: Specifies which modules to compile together,
and any extra flags needed for compiling these modules.  In the
simpler case, the value is an array of string specifying groups of
modules.  For example:
...
This means modules <tt>one</tt> and <tt>two</tt> will be compiled
together, then module three on its own, then any other modules
declared which are to be processed by Closure Compiler are created one
at a time.

If you need to use different flags for each compilation, you may
specify a hash for a compilation, for example:
...
Finally, to specify dependencies between modules, add a dependencies
key to the required modules:
...
For truly optimal results, compile all files that can exist on a page
together, with ADVANCED_OPTIMIZATIONS enabled.  This allows the
compiler to trim off unused code, and compress all the toplevel
symbols.  With judicious use of the <tt>compilations</tt> option, you
can control precisely how your javascripts are compiled.
